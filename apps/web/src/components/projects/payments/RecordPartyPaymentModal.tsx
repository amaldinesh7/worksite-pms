/**
 * Record Party Payment Modal
 *
 * Modal dialog for recording party payments (type = OUT).
 * Features:
 * - Party selection with outstanding amount display
 * - Pay against expense selection (optional)
...
export function RecordPartyPaymentModal({
  open,
  onOpenChange,
  projectId,
  payment,
}: RecordPartyPaymentModalProps) {
  ...
  // Fetch parties (exclude clients)
  const { data: partiesData } = useParties({
    limit: 100,
  });
  const parties = (partiesData?.items ?? []).filter((p) => p.type !== 'CLIENT');

  // Fetch outstanding amount for selected party
  const { data: outstandingData } = usePartyOutstanding(projectId, selectedPartyId);

  // Fetch unpaid expenses for selected party
  const { data: unpaidExpenses = [] } = usePartyUnpaidExpenses(projectId, selectedPartyId);
  ...
  // Update selected party when form changes
  useEffect(() => {
    setSelectedPartyId(watchedPartyId || '');
  }, [watchedPartyId]);
  ...
  useEffect(() => {
    if (open) {
      if (payment) {
        ...
        setSelectedPartyId(payment.partyId || '');
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Edit Payment' : 'Record Party Payment'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    {parties.map((party) => (
                      <SelectItem key={party.id} value={party.id} className="cursor-pointer">
          ...
          {selectedPartyId && outstandingData && (
          ...
          {/* Pay Against Expense (Optional) */}
          {selectedPartyId && unpaidExpenses.length > 0 && (
            <div className="space-y-2">
              <Label>Pay Against (Optional)</Label>
              <Controller
                name="expenseId"
                control={control}
                render={({ field }) => (
                  <Select value={field.value || ''} onValueChange={field.onChange}>
                    ...
                    <SelectContent>
                      {unpaidExpenses.map((expense) => (
                        <SelectItem key={expense.id} value={expense.id} className="cursor-pointer">
                          {expense.description} - Outstanding: {formatCurrency(expense.outstanding)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
