...
const ROLE_PERMISSIONS: Record<OrganizationRole, Permission[]> = {
  ADMIN: [
    { resource: 'projects', actions: ['create', 'read', 'update', 'delete', 'manage'], scope: 'all' },
    ...
    { resource: 'parties', actions: ['create', 'read', 'update', 'delete', 'manage'], scope: 'all' },
 ...
 */
export function requiresProjectAccess(role: OrganizationRole): boolean {
 ...
 */
export function hasFinancialAccess(role: OrganizationRole): boolean {
  return role === 'ADMIN' || role === 'MANAGER' || role === 'ACCOUNTANT';
}

// ============================================
// Role to PartyType Mapping
// ============================================

/**
 * Map PartyType to OrganizationRole for auto-assignment
 */
export function partyTypeToRole(
  partyType: string
): OrganizationRole | null {
  const mapping: Record<string, OrganizationRole> = {
...
/**
 * Check if a party type can have login capability
 */
export function canPartyTypeLogin(partyType: string): boolean {
  const loginableTypes = ['ACCOUNTANT', 'SUPERVISOR', 'PROJECT_MANAGER', 'CLIENT'];
  return loginableTypes.includes(partyType);
}
